...
--采矿头（需要是正方形）边长，整型，单位：格
mining_head_size = 3
--起重机杆朝向，字符串，选项为："north", "south", "west", "east"
direction = 'north'
--采矿头下挖等待时间，整型，单位：秒
mining_time = 5
--采矿头上升等待时间，整型，单位：秒
...
----------------------------
--选填参数：
--元件红石与电脑连接的方向，字符串，取决于 无线信号终端/红石线路 的摆放，选项为对于电脑六个面的："top","back","left","right","front","bottom"
...
--首轮采矿循环时，采矿头应该在相对于原点多少格后开始采矿，该值只能为mining_head_size的倍数，且只有第一轮生效，整型
--该参数一般用于从上次中断采矿的位置开始。例如，如果该值为3，那么采矿头将会从第4格开始沿着起重机杆采矿
--如需调整采矿框架的位置，请使用手摇曲柄
first_round_offset = 0

--本机通信频道，整型，用于在调制调解器网络中指定自己
local_modem_channel = 99
--无线通信频道，整型，用于向指定端口的调制调解器广播字符串
modem_channel = 0
...
function run(offset)
    ...
    for i = 1,loop_count do
        log("mining - (" .. tostring((i+offset)*mining_head_size) .. "/" .. tostring(gantry_shaft_length) .. ")")
...
function START()
    ...
    if first_round_offset >= mining_head_size then
        ...
        run(math.floor(first_round_offset/mining_head_size))
        log("end")
        log("----------------")
    end
